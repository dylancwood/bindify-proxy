...
describe('sendNewUserNotification', () => {
  ...
  const env = { SMTP2GO_API_KEY: 'test-key', ADMIN_NOTIFICATION_EMAIL: 'admin@example.com', OPS_NOTIFICATION_ORIGIN_EMAIL: 'Ops <ops@example.com>', SECRET_ENV_PREFIX: 'live' } as Env;
  ...
  it('prefixes subject with environment tag for non-production envs', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response('ok', { status: 200 }));
    const stagingEnv = { ...env, SECRET_ENV_PREFIX: 'stg' } as Env;

    await sendNewUserNotification(stagingEnv, user);

    const body = JSON.parse(fetchSpy.mock.calls[0][1]?.body as string);
    expect(body.subject).toBe('[STG] New Bindify user signed up');
  });
